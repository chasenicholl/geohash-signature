...
import shapely
from shapely.geometry import Polygon, LineString, shape as ShapelyShape
...
class TestGeohashSignature(unittest.TestCase):
    ...
    def setUpClass(cls):
        ...
        cls.poly_coords = [[[-73.99603330, 40.73283237],
        ...
        cls.polygon = Polygon(cls.poly_coords[0])
        cls.line_coords = [[-73.98718123, 40.73330603],
                           [-73.98577805, 40.73519710]]
        cls.line = LineString(cls.line_coords)
    ...
    def test_intersects(self):
        ...
        geohashes = geohash_signature.intersects(self.polygon, 11)
        self.assertEqual(len(geohashes),
                         24448,
                         'Geohash intersects count not correct')
        geohashes = geohash_signature.intersects(self.polygon, 10)
        self.assertEqual(len(geohashes),
                         821,
                         'Geohash intersects count not correct')
        geohashes = geohash_signature.intersects(self.line, 10)
        self.assertEqual(len(geohashes),
                         485,
                         'Geohash intersects count not correct')
    ...
    def test_within(self):
        ...
        geohashes = geohash_signature.within(self.polygon, 11)
        self.assertEqual(len(geohashes),
                         23634,
                         'Geohash within count not correct')
        geohashes = geohash_signature.within(self.polygon, 10)
        self.assertEqual(len(geohashes),
                         683,
                         'Geohash within count not correct')
    ...
    def test_compress_geohashes(self):
        ...
        prefix, components = geohash_signature.within(self.polygon,
                                                      10,
                                                      compress=True)
        ...
        prefix, components = geohash_signature.intersects(self.polygon,
                                                          10,
                                                          compress=True)
        self.assertEqual(prefix, 'dr5rspj')
